...
$ python train_dp.py --batch_size=32 --compile=False

To run with DDP on 4 GPUs on 1 node, example:
$ torchrun --standalone --nproc_per_node=4 train_dp.py

To run with DDP on 4 GPUs across 2 nodes, example:
- Run on the first (master) node with example IP 123.456.123.456:
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=0 --master_addr=123.456.123.456 --master_port=1234 train_dp.py
- Run on the worker node:
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=1 --master_addr=123.456.123.456 --master_port=1234 train_dp.py
...
target_epsilon = 5.0      # Desired epsilon
...
while True:
    ...
    # Always save checkpoint if it's time
    ...
    if (iter_num + 1) % gradient_accumulation_steps == 0:
        # Clip the gradients (handled by Opacus)
        if grad_clip != 0.0:
...
if privacy_engine_enabled:
    ...
    if master_process:
        print(f"Training completed with privacy budget: ε = {epsilon:.2f}, δ = {target_delta} for α = {best_alpha}")
